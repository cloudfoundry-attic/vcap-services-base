...
module VCAP
  module Services
    class CcngCatalogManager < VCAP::Services::CatalogManagerBase

          REQ_OPTS = %w(cloud_controller_uri service_auth_tokens token uaa_endpoint uaa_client_auth_credentials gateway_name logger).map {|o| o.to_sym}
          ...
          def initialize(opts)
            ...
            # Used for invoking list/update handles v1 api
            # TODO: remove this
            ...
            # TODO: Use custom client id from config
            client_id = "vmc"
            ti = CF::UAA::TokenIssuer.new(opts[:uaa_endpoint], client_id, "", nil)
          ...
          def update_catalog(activate, load_catalog_callback, after_update_callback = nil)
            f = Fiber.new do
              ...
              failed = false
              begin
                @catalog_in_ccdb = get_registered_services_from_cc
              rescue => e
                failed = true
          ...
          def advertise_services(active=true)
            ...
            new_offerings.each do |label|
              svc = @current_catalog[label]
              req, plans_to_add = generate_cc_advertise_offering_request(svc, active)
            ...
            @logger.info("CCNG Catalog Manager: Found #{active_count} active, #{disabled_count} disabled and #{new_offerings.size} new service offerings")
          ...
          def generate_cc_advertise_offering_request(svc, active = true)
            ...
            # The gateway provision request will have to maintain a mapping of plan-name to version so that
          ...
          def update_handle(service_label, handle, on_success_callback, on_failure_callback)
            @logger.debug("CC Catalog Manager: Update service handle: #{handle.inspect}")
            if not handle
              after_update_callback.call(false) if after_update_callback
              return
            end
