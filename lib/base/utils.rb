...
module VCAP::Services::Base::Utils
  ...
  def start_instances(all_instances)
    ...
    while start < all_instances.size
      instances = []
      if start + @instance_parallel_start_count < all_instances.size
        instances = all_instances.slice(start, @instance_parallel_start_count)
      else
        instances = all_instances.slice(start, all_instances.size - start)
      end
  ...
  def wait_server_start(instance)
    (@service_start_timeout * 10).times do
      sleep 0.1
      return true if is_server_started(instance)
    end
    false
  end

  def is_server_started(instance)
