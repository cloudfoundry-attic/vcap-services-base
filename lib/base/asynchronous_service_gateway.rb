...
require 'cc_catalog_manager'
require 'ccng_catalog_manager'
...
class VCAP::Services::AsynchronousServiceGateway < VCAP::Services::BaseAsynchronousServiceGateway
  ...
  def setup(opts)
    ...
    if @cc_api_version == "v1"
      @catalog_manager = VCAP::Services::CcCatalogManager.new(opts)
    else
      @catalog_manager = VCAP::Services::CcngCatalogManager.new(opts)
    end
  ...
  def get_current_catalog
    ...
    catalog_key = @service[:label]
    if @cc_api_version == "v2"
      catalog_key = "#{id}_#{@service[:provider] || 'core'}"
    end
