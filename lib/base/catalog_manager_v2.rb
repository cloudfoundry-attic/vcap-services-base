  ...
  module Services
    class CatalogManagerV2 < VCAP::Services::CatalogManagerBase
      ...
      def add_or_update_offering(offering, guid)
        ...
        req = create_http_request(:uri => uri, :method => method, :head => @cc_req_hdrs, :body => Yajl::Encoder.encode(offering)) do |http|
          if http.error.empty?
      ...
      def delete_offering(id, version, provider)
        ...
        req = create_http_request(:uri => uri, method => "delete", :head => @cc_req_hdrs) do |http|
          if http.error.empty?
            if (200..299) === http.response_header.status
