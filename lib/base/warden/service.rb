...
class VCAP::Services::Base::Warden::Service
  ...
  def run(options=nil, &post_start_block)
    ...
    bind_mounts = []
    options[:bind_dirs].each {|bind_dir| bind_mounts << bind_mount_request(bind_dir)}
