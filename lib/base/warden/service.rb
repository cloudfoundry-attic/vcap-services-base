...
class VCAP::Services::Base::Warden::Service
  ...
  class << self
    ...
    attr_reader :base_dir, :log_dir, :conf_dir, :image_dir, :bin_dir, :script_dir, :max_disk, :logger, :quota, :max_memory, :memory_overhead, :service_start_timeout, :bandwidth_per_second, :service_port, :rm_instance_dir_timeout
  end
  ...
  def run(options=nil, &post_start_block)
    ...
    bind_mounts = []
    options[:bind_dirs].each do |bind_dir|
      bind_mounts << bind_mount_request(bind_dir)
    end
  ...
  def bin_dir
    File.join(self.class.bin_dir, Dir.entries(self.class.bin_dir).find {|i| i.index(version)} || Dir.entries(self.class.bin_dir).find {|i| i.index(version.gsub(/\./, ""))})
  end
  ...
  def start_options
    bind_dirs = []
    bind_dirs << {:src => bin_dir}
    bind_dirs << {:src => File.join(self.class.bin_dir, "common")}
    bind_dirs << {:src => script_dir}
    bind_dirs << {:src => base_dir} if base_dir
    bind_dirs << {:src => log_dir} if log_dir
    bind_dirs << {:src => conf_dir} if conf_dir
    {
      :pre_start_script => {:script => "#{File.join(script_dir, "pre_service_start.sh")} #{base_dir} #{log_dir}", :use_root => true},
