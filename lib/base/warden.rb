...
module VCAP::Services::Base::Warden
  ...
  def run(opt_binds=[])
    ...
    unless opt_binds.empty?
      opt_binds.each do |opt_bind|
        new_bind = Warden::Protocol::CreateRequest::BindMount.new
        new_bind.src_path = opt_bind[:src_path]
        new_bind.dst_path = opt_bind[:dst_path]
        new_bind.mode     = opt_bind[:mode]
        bind_mounts << new_bind
      end
    end
