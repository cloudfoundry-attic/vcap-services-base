  ...
  module Services
    module ExternalServices
      class Gateway < VCAP::Services::BaseAsynchronousServiceGateway
        ...
        def load_external_service_gateway(opts)
          external_service_gateway_lib_path = opts[:external_service_gateway_lib_path]
          @logger.info("Loading external service gateway from: #{external_service_gateway_lib_path}")

          $LOAD_PATH.unshift(external_service_gateway_lib_path)
          Dir[external_service_gateway_lib_path + '/*.rb'].each do |file|
            f = File.basename(file, File.extname(file))
            require f
          end

          # To minimize the amount of external_service_gateway-specific code, the config file specifies the class that
          # implements MarketplaceBase's abstract methods for this external_service_gateway. So we need to translate the
        ...
        def setup(opts)
          ...
          @ext_service_gw_client = load_external_service_gateway(opts)
          ...
          @router_register_json = {
            ...
            :tags => {:components => "#{@ext_service_gw_client.name} Service Gateway" }
          }.to_json

          @catalog = {}

          @external_service_gateway_gateway_varz_details = {}
          @stats = {}
          @stats_lock = Mutex.new

          opts[:cloud_controller_uri]    = http_uri(opts[:cloud_controller_uri] || "api.vcap.me")
          opts[:gateway_name] = @ext_service_gw_client.name
          ...
          @catalog_manager.update_catalog(
            ...
            lambda {
              Timeout::timeout(@node_timeout) do
                return @ext_service_gw_client.get_catalog
              end
            },
            lambda {
              update_varz
        ...
        def update_varz
          ...
          VCAP::Component.varz[@ext_service_gw_client.name.downcase] = @ext_service_gw_client.varz_details if @ext_service_gw_client.varz_details.size > 0
        end
        ...
        def start_nats(uri)
          ...
          @nats = NATS.connect(:uri => uri) do
            VCAP::Component.register(
              :nats     => @nats,
              :type     => "#{@ext_service_gw_client.name} Service Gateway",
              :host     => @component_host,
              :port     => @component_port,
              :index    => @index,
              :user     => @component_user,
        ...
        def on_connect_nats()
          @logger.info("Register #{@ext_service_gw_client.name} service gateway: #{@router_register_json}")
        ...
        def stop_nats()
          ...
          @logger.debug("Unregister #{@ext_service_gw_client.name} service gateway: #{@router_register_json}")
        ...
        # Provision a external_service_gateway service
          ...
          Fiber.new{
            ...
            begin
              msg = Timeout::timeout(@node_timeout) do
                @ext_service_gw_client.provision_service(request_body)
              end
        ...
        post "/gateway/v1/configurations/:service_id/handles" do
          ...
          Fiber.new {
            ...
            begin
              msg = Timeout::timeout(@node_timeout) do
                @ext_service_gw_client.bind_service_instance(params['service_id'], req.binding_options)
              end
              resp = VCAP::Services::Api::GatewayHandleResponse.new(msg).encode
              async_reply(resp)
            rescue => e
        ...
        delete "/gateway/v1/configurations/:service_id" do
          ...
          Fiber.new {
            ...
            begin
              Timeout::timeout(@node_timeout) do
                @ext_service_gw_client.unprovision_service(sid)
              end
              async_reply
            rescue => e
              failed = true
        ...
        delete "/gateway/v1/configurations/:service_id/handles/:handle_id" do
          ...
          Fiber.new {
            ...
            begin
              Timeout::timeout(@node_timeout) do
                @ext_service_gw_client.unbind_service(sid, hid)
              end
              async_reply
            rescue => e
              failed = true
