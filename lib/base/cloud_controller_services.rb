  ...
  class CloudControllerServices
    ...
    def load_registered_services(service_list_uri, auth_token_registry)
      ...
      self.each(service_list_uri, "Registered Offerings") do |s|
        label, _, version = s["entity"]["label"].rpartition('-')
        provider = s["entity"]["provider"]
        key = "#{label}_#{provider}"

        unless auth_token_registry.has_key?(key.to_sym)
          logger.debug("Ignoring service #{label} from provider #{provider}, because it has no auth token registered")
          next
        end
