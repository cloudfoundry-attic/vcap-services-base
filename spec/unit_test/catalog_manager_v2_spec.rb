...
module VCAP::Services
  describe CatalogManagerV2 do
    ...
    describe "#update_catalog" do
      ...
      let(:gateway_service_catalog) { double('gateway service catalog') }
      let(:catalog_loader) { ->{ gateway_service_catalog } }
      let(:registered_services) { mock('registered service', load_registered_services: {}) }

      before do
        CloudControllerServices.stub(:new => registered_services)
        ServiceAdvertiser.stub(:new => null_object)
      end
      ...
      it "creates a service advertiser" do
        ServiceAdvertiser.should_receive(:new).
          with(hash_including(current_catalog: gateway_service_catalog)).
